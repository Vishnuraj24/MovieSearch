...
using MovieSearch.Movies.Api.Kafka;
...
namespace MovieSearch.Movies.Api.Controllers
{
    ...
    public sealed class MoviesController : ControllerBase
    {
        ...
        [HttpPost("api/movies")]
        public async Task<IActionResult> Create([FromBody] Movie movie, [FromServices] IMovieEventProducer producer)
        {
            // Instead of calling _svc.IndexAsync(movie), publish an event
            var evt = new
            {
                type = "MovieUpserted",
            ...
            await producer.ProduceUpsertAsync(evt);
            return Accepted(new { message = "Event published", movie.Id });
        }
